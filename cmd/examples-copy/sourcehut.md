...
> Note: all command line arguments can be permanently set in your `$HOME/.config/ghorg/conf.yaml` for more information see the [configuration](https://github.com/blairham/ghorg#configuration) section of the README.md.

To view all additional flags see the [sample-conf.yaml](https://github.com/blairham/ghorg/blob/master/sample-conf.yaml) or use `ghorg clone --help`
...
4. **Username format**: You can provide usernames with or without the `~` prefix (e.g., both `blairham` and `~blairham` work). Local folder paths will never include the `~` prefix to avoid shell expansion issues.
